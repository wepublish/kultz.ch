...
import {RightArrow, LeftArrow, SliderList, SliderTrack, SliderWrapper,CardWrapper,DefaultDotsWrapper,DotButton} from './Components'
...
class Slider extends React.Component {
  ...
  componentDidMount() {
    const {children, cardsToShow: cardsToShowProp, autoSlide, hideArrowsOnNoSlides} = this.props
  ...
  updateResponsiveView() {
    ...
    if (responsive) {
      responsive = responsive
        ...
        .sort((key => (a, b) => (a[key] > b[key] ? 1 : b[key] > a[key] ? -1 : 0))('breakPoint')) // eslint-disable-line
    ...
    for (i = 0; i <= numberOfChildren - this.state.cardsToShow; i += 1) {
      ...
      dots.push(
        <DotButton
          ...
          onClick={this.changeInitialCard.bind(this, index)}>
          {`•`}
        </DotButton>
      )
    }
  ...
  renderRightArrow() {
    ...
    if (initialCard + cardsToShow < numberOfChildren)
      ...
      <div
        ...
        <SliderWrapper {...otherProps}>
          ...
          <div
            ...
            <div>
              {showArrows && !this.state.hideArrows && <div>{this.renderLeftArrow()} </div>}
            </div>
            <DotsWrapper>{showDots && this.renderDots()}</DotsWrapper>
            {showArrows && !this.state.hideArrows && <div>{this.renderRightArrow()}</div>}
          </div>
        </SliderWrapper>
      </div>
    )
...
//   DotsWrapper: PropTypes.oneOfType([PropTypes.func, PropTypes.object])
// }
